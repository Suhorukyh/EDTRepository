...
Процедура ВыбратьВыполнить()
	...
	//КонецЕсли;
	//Оповестить("ВыборОбъектовМетаданных", ВыбранныеОбъектыМетаданных, Параметры.УникальныйИдентификаторИсточник);
	Оповестить("ВыборОбъектовМетаданных", ВыбранныеОбъектыМетаданных);
	Закрыть(ВыбранныеОбъектыМетаданных)
...
//
&НаСервере
Функция ВыводКоллекцииОбъектовМетаданных(Имя, ПолноеИмя, Синоним, Картинка, КартинкаОбъекта, Родитель = Неопределено, Подсистемы = Неопределено)
	...
	Если Подсистемы = Неопределено Тогда
		Для каждого ЭлементКоллекцииМетаданных ИЗ Метаданные[Имя] Цикл
			// ++ chernav {
			если не ПравоДоступа("ИнтерактивноеУдалениеПомеченных", ЭлементКоллекцииМетаданных) тогда
				продолжить;
			иначеесли не ОбменДаннымиСервер.РазрешеноИзменениеДанных(ЭлементКоллекцииМетаданных) тогда
				продолжить;
			конецесли;
			// ++ chernav }
...
Процедура НачальнаяПометкаКоллекций(Родитель = Неопределено)
	...
	Если НЕ Родитель.ЭтоОбъектМетаданных Тогда
		...
		ИначеЕсли ВложенныеЭлементы[0].ЭтоОбъектМетаданных Тогда
			ПометитьЭлементыРодителей(ВложенныеЭлементы[0]);
		Иначе
